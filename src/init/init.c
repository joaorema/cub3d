...
void init_wall(t_game *game, t_rayhit *vhit, t_rayhit *hhit, t_wall *wall, float angle)
{
    ...
    float corrected_distance = wall->distance * cos(angle - game->player_angle);
    ...
    float offset = wall->hit_vertical 
                    ? fmodf(wall->hit_point.y + 0.0001f, TILE_SIZE)
                    : fmodf(wall->hit_point.x + 0.0001f, TILE_SIZE);
